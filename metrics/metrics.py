...
def metrics_for_image_superresolution(config, epoch):
    def metrics(predictions, targets):
        return_metrics =  {
            ...
            'cc_4': cross_correlation(1)(predictions, targets),
            'cc_8': cross_correlation(2)(predictions, targets),
            'cc_16': cross_correlation(3)(predictions, targets),
        }
        if epoch >= config.JI_metrics_from_epoch:
            ji, rmse, efficiency = get_ji_rmse_efficiency_from_predictions(3, config)(predictions, targets)
