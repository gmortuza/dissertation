...
class Conv2d_batchnorm(nn.Module):
    def __init__(self, input_features: int, num_of_filters: int, kernel_size: Tuple = (2, 2), stride: Tuple = (1, 1),
        ...
        self.conv1 = nn.Conv2d(in_channels=input_features, out_channels=num_of_filters, kernel_size=kernel_size,
...
class MultiResUNet(nn.Module):
    def __init__(self, config, in_channel: int = 1, out_channel: int = 1, filter_starts: int = 32) -> None:
        ...
        # Decoder path
        self.upsample6 = nn.ConvTranspose2d(in_channels=self.in_filters5, out_channels=self.filters * 8,
        ...
        self.in_filters6 = int(self.filters * 8 * self.alpha * 0.5) + int(self.filters * 8 * self.alpha * 0.167) + int(
            self.filters * 8 * self.alpha * 0.333)
        self.upsample7 = nn.ConvTranspose2d(in_channels=self.in_filters6, out_channels=self.filters * 4,
                                            kernel_size=(2, 2), stride=(2, 2), padding=0)
        ...
        self.in_filters7 = int(self.filters * 4 * self.alpha * 0.5) + int(self.filters * 4 * self.alpha * 0.167) + int(
            self.filters * 4 * self.alpha * 0.333)
        self.upsample8 = nn.ConvTranspose2d(in_channels=self.in_filters7, out_channels=self.filters * 2,
                                            kernel_size=(2, 2), stride=(2, 2), padding=0)
        ...
        self.in_filters8 = int(self.filters * 2 * self.alpha * 0.5) + int(self.filters * 2 * self.alpha * 0.167) + int(
            self.filters * 2 * self.alpha * 0.333)
        self.upsample9 = nn.ConvTranspose2d(in_channels=self.in_filters8, out_channels=self.filters, kernel_size=(2, 2),
